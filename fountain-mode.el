...
expanded.
  ...
  %t    Current date and time as defined in `fountain-time-format'
  %U    User full name (value of `user-full-name')
  %u    User login name (capitlized value of `user-login-name')
  %e    User email (value of `user-mail-address')
  %i    UUID generated by CLT uuidgen
  %s    Entry text, either the selected region or entered"
  :type 'string
  :group 'fountain)
...
If prefixed with ARG, only insert note delimiters (\"[[\" \"]]\")."
  ...
  (let* ((comment-start "[[")
         (comment-end "]]")
         (entry (if (use-region-p)
                    (buffer-substring-no-properties
                     (region-beginning) (region-end))
                  (read-string "Note: ")))
         (str (format-spec fountain-note-template
