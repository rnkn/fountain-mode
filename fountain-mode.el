...
(defcustom fountain-note-template
  "%t - %U: "
  "Template used when inserting note elements.

The following %-escapes will be replaced with content and
expanded.

  %t    Current date and time as defined in `fountain-short-time-format'
  %T    Current date and time as defined in `fountain-long-time-format'
  %u    User first name (capitlized value of `user-login-name')
  %U    User full name (value of `user-full-name')
  %e    User email (value of `user-mail-address')
  %i    UUID (generated by `fountain-uuid-fuction')

Therefore the default (\"%t - %U: \") will insert something like:
...
If prefixed with ARG, only insert note delimiters (\"[[\" \"]]\")."
  ...
  (let ((comment-start "[[")
    ...
    (if arg
        (comment-dwim nil)
      (let ((str (format-spec fountain-note-template
                              `((?t . ,(format-time-string fountain-time-format))
                                (?U . ,user-full-name)
                                (?u . ,(capitalize user-login-name))
                                (?e . ,user-mail-address)
                                (?i . ,(fountain-uuid))))))
